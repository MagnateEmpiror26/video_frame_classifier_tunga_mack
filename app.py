...
def predict():
    ...
    def giveResults(images_array, decoded_response):
        search_query_value = request.form['name_of_object']
        image_index = 0
        for i in range(len(decoded_response)):
            class_tupple = decoded_response[i]
            _id, frame_class, frame_prob = class_tupple[0]
            image_index = i
            print(image_index, frame_class)
    ...
    image_index, frame_class = giveResults(query_frames_array,decoded_query_results)
